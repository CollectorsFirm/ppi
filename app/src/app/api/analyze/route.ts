...
- TRANSMISSION PREFERENCES: On air-cooled Porsches (964, 993), Tiptronic is a meaningful value detractor versus manual — flag it as a watchOut (not a red flag). On water-cooled Porsches and modern cars, automatic vs manual is a preference not a concern unless the listing is for a GT car where manual is expected.
...
- NOTABLE PREVIOUS OWNERS: If a car was previously owned by a celebrity, professional athlete, racing driver, or other notable figure — treat this as a genuine green flag and provenance asset. Any signature, autograph, or personalization from the notable owner (on the dash, door sill, etc.) is part of the provenance and should be treated as a positive. NEVER flag a signature or autograph as a concern on a provenance car. Mention who they are and why it matters. Factor it into the fairMarketEstimate as a premium (notable provenance typically adds 5–20% on BaT depending on how well-known the person is and how relevant they are to the car).
...
const BANNED_FLAG_PATTERNS = [
  ...
  /signature.*(concern|damage|removal|buff|marker|permanent)/i,
  /autograph.*(concern|damage|removal|buff|marker|permanent)/i,
  /(buff|remov).*(signature|autograph)/i,
];
